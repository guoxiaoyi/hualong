namespace :admin do
  ...
  task :init_manager => :environment do
    @manager = Manager.new( name: 'admin', email: 'admin@admin.com', password: 'admin888', password_confirmation: 'admin888', supper: true, disabled: false)
    @manager.save
  ...
  task :reset_manager => :environment do
    @manager = Manager.where(name: 'admin', email: 'admin@admin.com').first
    @manager.update_attributes(supper: true, disabled: false)
